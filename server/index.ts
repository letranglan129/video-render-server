...
function setupApp({ remotionBundleUrl }: { remotionBundleUrl: string }) {
  ...
  app.post("/renders", async (req, res) => {
    const { tracks, durationInFrames, fps, height, width } = req.body;

    if (!tracks || !durationInFrames || !fps || !height || !width) {
      res.status(400).json({ message: "`tracks`, `durationInFrames`, `fps`, `height`, and `width` are required" });
      return;
    }

    const jobId = queue.createJob({ tracks, durationInFrames, fps, height, width });
